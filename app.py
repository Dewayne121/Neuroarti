...
# --- Image Engine (Unchanged from previous working version) ---
...
def find_image_from_pexels(query: str) -> str:
    ...
    except Exception as e: print(f"Pexels API error for query '{query}': {e}")
    return ""
...
def fix_image_sources_smart(html_content: str) -> str:
    ...
    try:
        ...
        for img in images:
            ...
            if not image_url: image_url = random.choice(get_hardcoded_fallback_images())
            img['src'] = image_url
...
# --- THE NEW HIGH-QUALITY CREATIVE BRIEF ---
SUPERCHARGED_DESIGN_BRIEF = (
    "You are 'Vortex,' an elite AI frontend architect renowned for creating visually stunning, modern, and award-winning websites. Your designs are clean, sophisticated, and highly engaging. You will generate a single, complete HTML file using Tailwind CSS via a CDN link.\n\n"
    ...
    "    - **Varied Layouts:** Each `<section>` must have a different and interesting layout. Do not just stack text blocks. Use alternating image-and-text sections, centered content, and multi-column grids.\n\n"
    "2.  **Color & Typography:**\n"
    ...
    "    - **Typographic Hierarchy:** Use a clear hierarchy. Large, bold headings (`text-4xl`, `font-bold`), smaller subheadings, and legible body text (`text-lg`, `text-slate-300`). Use different font weights (`font-light`, `font-medium`).\n"
    "    - **High Contrast is NON-NEGOTIABLE:** All text must be easily readable against its background.\n\n"
    "3.  **Components & Interactivity:**\n"
    "    - **Rich Component Library:** Your page MUST include several of these: a compelling hero section with a clear call-to-action button, a multi-item feature grid with inline SVG icons, a customer testimonial section (use blockquotes), a pricing table, and a detailed contact form.\n"
    "    - **Subtle Polish:** Use smooth transitions on interactive elements (`transition-all`, `duration-300`). Add subtle hover effects like lifting up (`hover:-translate-y-1`), growing (`hover:scale-105`), or changing color/shadow (`hover:shadow-lg`).\n"
    "    - **Engaging Buttons:** Buttons must be visually appealing with padding, a background color, and hover effects.\n\n"
    "4.  **Content & Images:**\n"
    "    - **Fill with Realistic Content:** Generate plausible and relevant headlines, paragraphs, and list items. The page should look complete and professional.\n"
    ...
    "- Your entire response is ONLY the raw HTML code.\n"
    "- Start immediately with `<!DOCTYPE html>`. No explanations, no markdown, no apologies.\n"
    "- Include Tailwind CSS via `<script src=\"https://cdn.tailwindcss.com\"></script>` in the `<head>`.\n"
    "- All custom CSS goes inside a `<style>` tag in the `<head>`.\n"
    "- All JS goes inside a `<script>` tag before the closing `</body>` tag.\n"
)

# --- AI Core Functions (Updated with new temperature) ---
...
def generate_with_google(system_prompt: str, user_prompt: str, model_id_str: str):
    ...
    generation_config = genai.types.GenerationConfig(temperature=0.3) # Increased for more creativity
...
# --- API Endpoints (Using the new brief) ---
@app.post("/build")
