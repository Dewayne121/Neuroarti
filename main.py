...
async def rewrite_element_endpoint(request: Request, body: RewriteRequest):
    ...
    try:
        ...
        # Parse the selected element HTML to extract its properties
        selected_soup = BeautifulSoup(body.selectedElementHtml, 'html.parser')
        selected_element = selected_soup.find()
        
        if not selected_element:
            raise Exception("Invalid selected element HTML.")
        
        # Create a more flexible search using tag name, attributes, and text content
        def find_matching_element(tag):
            if not isinstance(tag, Tag) or tag.name != selected_element.name:
                return False
            
            # Compare attributes (normalize classes for comparison)
            for attr, value in selected_element.attrs.items():
                if attr == 'class':
                    # Compare classes as sorted lists to handle order differences
                    if sorted(tag.get(attr, [])) != sorted(value):
                        return False
                else:
            ...
            if tag.get_text(strip=True) != selected_element.get_text(strip=True):
                return False
            
            return True
        
        target_element = full_soup.find(find_matching_element)
