...
def clean_ai_response(raw_text: str) -> str:
    ...
    # First, try to find a markdown code block, as it's the most likely format.
    # This regex is designed to be flexible with or without the 'html' language identifier.
    markdown_match = re.search(r'```(?:html)?\s*(<.*?>.*</.*?>)\s*```', raw_text, re.DOTALL | re.IGNORECASE)
    if markdown_match:
    ...
    try:
        ...
        # As a last resort for malformed output, use a simpler regex for any tag structure.
